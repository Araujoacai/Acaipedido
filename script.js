...
// A 'DOMContentLoaded' listener ensures the script runs after the HTML is fully loaded.
document.addEventListener('DOMContentLoaded', () => {
    ...
    function limparSelecaoAcompanhamentos() {
        ...
        const apenasAcaiCheck = document.getElementById('apenas-acai-check');
        if (apenasAcaiCheck) {
            apenasAcaiCheck.checked = false;
        }
    }
    ...
    if (adicionarCopoBtn) {
        adicionarCopoBtn.addEventListener('click', () => {
            ...
            const apenasAcaiCheck = document.getElementById('apenas-acai-check');
            const apenasAcai = apenasAcaiCheck ? apenasAcaiCheck.checked : false;
            ...
            const regraAcompanhamentos = storeSettings.regraAcompanhamentos || { porcoesGratis: 3, precoAdicional: 3 };

            if (!apenasAcai && totalPorcoes > regraAcompanhamentos.porcoesGratis) {
                 adicionais = (totalPorcoes - regraAcompanhamentos.porcoesGratis) * regraAcompanhamentos.precoAdicional;
            }
    ...
    function calcularValorTotal() {
        ...
        const valorMobile = document.getElementById("valor-mobile");
        const valorDesktop = document.getElementById("valor-desktop");
        if(valorMobile) valorMobile.innerText = totalText;
        if(valorDesktop) valorDesktop.innerText = totalText;
    }
    ...
    async function enviarPedido() {
        // ... (restante da função enviarPedido)
    }
    ...
    async function salvarCombo() {
        ...
        if (!combo.name || !combo.price) { showModal("Nome e Preço são obrigatórios para o combo."); return; }
        try {
            if (id) { 
            ...
            <div class="bg-white p-6 rounded-2xl shadow-lg mb-8">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left">
                        <thead class="bg-gray-100"><tr><th class="p-3">ID</th><th class="p-3">Data</th><th class="p-3">Cliente</th><th class="p-3">Pedido</th><th class="p-3">Financeiro</th><th class="p-3">Status</th><th class="p-3">Ações</th></tr></thead>
                        <tbody id="vendas-table-body"></tbody>
                    </table>
                </div>
                <div class="mt-4 text-right"><h4 class="text-xl font-bold">Total: <span id="total-vendas">R$0,00</span></h4></div>
            </div>`;
        carregarVendasAdmin();
    }
    ...
    // --- Seção de Fluxo de Caixa (IMPLEMENTADA) ---
        ...
        contentEl.innerHTML = `
            <div class="bg-white p-6 rounded-2xl shadow-lg mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 p-4 border rounded-lg">
                    ...
                    <input type="text" id="transacao-descricao" placeholder="Descrição" class="p-2 border rounded md:col-span-2">
                    ...
                    <button id="salvar-transacao-btn" class="bg-blue-500 text-white p-2 rounded hover:bg-blue-600 md:col-span-4">Adicionar Lançamento</button>
                </div>
                <div class="grid grid-cols-3 gap-4 mb-4 text-center">
                    <div class="bg-green-100 p-4 rounded-lg"><h4 class="font-semibold">Entradas</h4><p id="total-entradas" class="text-lg font-bold text-green-700">R$0,00</p></div>
                    <div class="bg-red-100 p-4 rounded-lg"><h4 class="font-semibold">Saídas</h4><p id="total-saidas" class="text-lg font-bold text-red-700">R$0,00</p></div>
                    <div class="bg-blue-100 p-4 rounded-lg"><h4 class="font-semibold">Saldo</h4><p id="saldo-atual" class="text-lg font-bold text-blue-700">R$0,00</p></div>
                </div>
                <div class="overflow-x-auto"><table class="w-full text-sm text-left"><thead class="bg-gray-100"><tr><th class="p-3">Data</th><th class="p-3">Descrição</th><th class="p-3">Tipo</th><th class="p-3">Valor</th><th class="p-3">Ações</th></tr></thead><tbody id="caixa-table-body"></tbody></table></div>
            </div>`;
        document.getElementById('salvar-transacao-btn').addEventListener('click', salvarTransacao);
        carregarFluxoCaixa();
    }
    
    // --- Seção de Configurações (IMPLEMENTADA) ---
    function renderConfigAdmin() {
        const contentEl = document.getElementById('content-config');
        if(!contentEl) return;
        let diasSemanaHTML = ['domingo', 'segunda', 'terca', 'quarta', 'quinta', 'sexta', 'sabado'].map(dia => `
            <div class="grid grid-cols-4 items-center gap-2 border-b py-2 capitalize">
                <label class="font-semibold">${dia}</label>
                <input type="checkbox" id="${dia}-aberto" class="justify-self-center h-5 w-5">
                <input type="time" id="${dia}-abertura" class="p-1 border rounded">
                <input type="time" id="${dia}-fechamento" class="p-1 border rounded">
            </div>`).join('');

        contentEl.innerHTML = `
            <div class="bg-white p-6 rounded-2xl shadow-lg mb-8">
                <h3 class="text-2xl font-semibold mb-4 text-purple-700">Configurações da Loja</h3>
                <div class="space-y-4">
                    <div><label class="font-semibold block mb-1">Número do WhatsApp (com 55)</label><input type="text" id="whatsapp-number" class="w-full p-2 border rounded"></div>
                    <div><label class="font-semibold block mb-1">Mensagem (Loja Fechada)</label><textarea id="mensagem-fechado" class="w-full p-2 border rounded" rows="3"></textarea></div>
                    <div>
                        <h4 class="text-lg font-semibold mt-4 mb-2">Horário de Funcionamento</h4>
                        <div class="grid grid-cols-4 gap-2 font-bold text-sm text-center mb-2 px-2"><span class="col-start-2">Aberto?</span><span>Abertura</span><span>Fechamento</span></div>
                        ${diasSemanaHTML}
                    </div>
                    <button id="salvar-config-btn" class="bg-green-500 text-white p-3 rounded-lg hover:bg-green-600 w-full mt-6">Salvar Configurações</button>
                </div>
            </div>`;
