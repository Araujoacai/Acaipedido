...
let produtos = [];
let pedidoAtual = []; // Armazena os copos do pedido atual
    ...
    showModal(loginFormHTML, () => {
        document.getElementById('login-submit').addEventListener('click', async () => {
            try { await signInWithEmailAndPassword(auth, document.getElementById('email').value, document.getElementById('password').value);
                closeModal(); } catch (error) { console.error("Erro de login:", error);
...
function renderMenu() {
    ...
    document.querySelectorAll('.acompanhamento-check').forEach(check => {
        check.addEventListener('change', (e) => {
            ...
            // Não recalcula o valor total aqui, pois ele depende do pedido
        });
    });
    ...
    document.getElementById('add-cup-button').addEventListener('click', adicionarCopo);
}
...
function calcularValor() {
    ...
    pedidoAtual.forEach(copo => {
        ...
        totalPedido += (precoBase + adicionais);
    });
...
function renderPedidoAtual() {
    const container = document.getElementById('pedido-atual-container');
    if (pedidoAtual.length === 0) {
        container.innerHTML = '<p class="text-gray-500">Nenhum copo adicionado ainda.</p>';
        return;
    }

    container.innerHTML = '';
    pedidoAtual.forEach((copo, index) => {
        const acompanhamentosText = copo.acompanhamentos.map(a => `${a.name} (x${a.quantity})`).join(', ') || 'Nenhum';
        container.innerHTML += `
            <div class="border p-4 rounded-lg bg-purple-50">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="font-bold text-purple-800">Copo ${index + 1}: ${copo.tamanho}</p>
                        <p class="text-sm text-gray-700"><b>Acompanhamentos:</b> ${acompanhamentosText}</p>
                        ${copo.observacoes ? `<p class="text-sm text-blue-600"><b>Obs:</b> ${copo.observacoes}</p>` : ''}
                    </div>
                    <div>
                        <button onclick="window.editarCopo(${copo.id})" class="p-1 text-blue-500" title="Editar">✏️</button>
                        <button onclick="window.removerCopo(${copo.id})" class="p-1 text-red-500" title="Remover">🗑️</button>
                    </div>
                </div>
            </div>
        `;
...
function removerCopo(id) {
    pedidoAtual = pedidoAtual.filter(copo => copo.id !== id);
    renderPedidoAtual();
    calcularValor();
}

function editarCopo(id) {
    const copoParaEditar = pedidoAtual.find(copo => copo.id === id);
    if (!copoParaEditar) return;

    document.querySelector(`input[name="tamanho"][value="${copoParaEditar.tamanho}"]`).checked = true;
    document.getElementById('apenas-acai-check').checked = copoParaEditar.apenasAcai;
    document.getElementById('observacoes').value = copoParaEditar.observacoes;
    ...
    copoParaEditar.acompanhamentos.forEach(acomp => {
    ...
    removerCopo(id);
    showModal("Edite o copo e clique em 'Adicionar Copo ao Pedido' para salvar as alterações.");
}

window.removerCopo = removerCopo;
window.editarCopo = editarCopo;
...
async function enviarPedido() {
    ...
    if (pedidoAtual.length === 0) {
        showModal("Por favor, adicione pelo menos um copo ao seu pedido!");
        return;
    }
    ...
    try {
        await addDoc(collection(db, "vendas"), {
            ...
            nomeCliente,
            telefoneCliente,
            pedidoDetalhado: pedidoAtual,
            quantidade: pedidoAtual.length,
            total: valor,
            status: "pendente",
...
async function salvarCombo() {
    ...
    try {
        ...
        document.getElementById('combo-imagem').value = '';
    } catch (error) { console.error("Erro ao salvar combo:", error);
        showModal("Não foi possível salvar o combo."); }
}
...
function deletarCombo(id) {
    showModal(`<h3 class="text-xl font-bold mb-4">Confirmar Exclusão</h3><p class="mb-6">Tem certeza que deseja excluir este combo?</p><button id="confirm-delete-combo-btn" class="bg-red-500 text-white px-6 py-2 rounded-lg">Excluir</button><button onclick="window.closeModal()" class="bg-gray-300 px-4 py-2 rounded-lg ml-2">Cancelar</button>`, () => {
        document.getElementById('confirm-delete-combo-btn').addEventListener('click', async () => {
            try { await deleteDoc(doc(db, "combos", id));
                closeModal(); } catch (error) { console.error("Erro ao excluir combo:", error);
                closeModal();
                showModal('Ocorreu um erro ao excluir o combo.'); }
        });
    });
}
...
async function toggleComboStatus(id) {
    ...
    if (combo) {
        ...
        try { await updateDoc(doc(db, "combos", id), { isActive: newStatus }); } catch (error) { console.error("Erro ao atualizar status:", error);
            showModal("Não foi possível atualizar o status do combo."); }
    }
}
    ...
    showModal(confirmationHTML, () => {
        document.getElementById('confirm-delete-produto-btn').addEventListener('click', async () => {
            try { await deleteDoc(doc(db, "produtos", id));
                closeModal(); } catch (error) { console.error("Erro ao excluir produto:", error);
                closeModal();
                showModal('Ocorreu um erro ao excluir o produto.'); }
        });
    });
}
...
function calcularCustoPedido(venda) {
    ...
    if (venda.pedidoDetalhado && venda.pedidoDetalhado.length > 0) {
        venda.pedidoDetalhado.forEach(copo => {
            ...
            if (copo.acompanhamentos) {
                 copo.acompanhamentos.forEach(itemPedido => {
                    ...
                    if (acompanhamentoProduto) {
                        custoTotal += (itemPedido.quantity || 1) * (acompanhamentoProduto.cost || 0);
                    }
                });
            }
        });
    ...
    unsubscribeVendas = onSnapshot(q, (snapshot) => {
        ...
        if (snapshot.empty) {
            ...
            snapshot.docs.forEach(docSnap => {
                ...
                if (venda.pedidoDetalhado && venda.pedidoDetalhado.length > 0) {
                    venda.pedidoDetalhado.forEach((copo, index) => {
                        pedidoHTML += `<div class="mb-1 ${venda.pedidoDetalhado.length > 1 ? 'border-b pb-1' : ''}"><b>Copo ${index + 1}:</b> ${copo.tamanho}<br>`;
                        pedidoHTML += `<small class="text-gray-500">${(copo.acompanhamentos || []).map(a => `${a.name}(x${a.quantity})`).join(', ')}</small><br>`;
                        if (copo.observacoes) {
                            pedidoHTML += `<small class="text-blue-500 font-semibold">Obs: ${copo.observacoes}</small></div>`;
                        }
                    });
...
function deletarVenda(id) {
    ...
    showModal(confirmationHTML, () => {
        document.getElementById('confirm-delete-btn').addEventListener('click', async () => {
            try { await deleteDoc(doc(db, "vendas", id));
                closeModal(); } catch (error) { console.error("Erro ao excluir venda:", error);
                closeModal();
...
async function salvarConfiguracoes() {
    ...
    try {
        ...
        showModal('Erro ao salvar as configurações.'); }
}
...
async function salvarTransacao() {
    ...
    try {
        if (id) { await updateDoc(doc(db, "fluxoCaixa", id), { descricao: transacao.descricao, valor: transacao.valor, tipo: transacao.tipo }); } else { await addDoc(collection(db, "fluxoCaixa"), transacao); }
        ...
        showModal("Não foi possível salvar a transação."); }
}
...
function deletarTransacao(id) {
    ...
    showModal(confirmationHTML, () => {
        document.getElementById('confirm-delete-transacao-btn').addEventListener('click', async () => {
            try { await deleteDoc(doc(db, "fluxoCaixa", id));
                closeModal(); } catch (error) { console.error("Erro ao excluir transação:", error);
                closeModal();
                showModal('Ocorreu um erro ao excluir.'); }
        });
    });
}
